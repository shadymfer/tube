...
export function Vaults({ mutationData, takerBankWhitelist, connection, wallet, selectedTokens, setSelectedTokens }: VaultProps) {
	...
	return (
		<>
			...
			<div className="pb-20 text-gray-900 flex justify-center opacity-80 rounded-lg">
				{/* TAKER VAULTS */}
				<div className="flex lg:flex-row lg:gap-x-32 flex-col gap-x-2 w-9/12 opacity-100 justify-center bg-gray-900 pb-20 rounded-lg px-4">
					{Object.keys(mutation).map((key, index) => {
							...
							return (
								<div key={key}>
									{key.includes("takerToken") && mutation[key].requiredAmount.toNumber() > 0 && (
										<div>
											<div className="py-5 border-b border-gray-200 opacity-100 justify-center">
												<h3 className="text-lg leading-1 font-medium text-gray-50 opacity-100">Tube {key.split("takerToken")[1]}</h3>
												<div className="flex items-center text-md mt-2 ">
													{/* <TrendingDownIcon className="h-5 w-5 text-gray-400" aria-hidden="true" /> */}
													<span className="text-gray-50 pl-1">🧬 Put</span>
													<span className="text-gray-50 font-medium- font-bold pl-1.5">{mutation[key].requiredAmount.toNumber()}</span>
													<span className="text-gray-50 pl-1">token{mutation[key].requiredAmount.toNumber() > 1 && "s"} ingredient</span>
												</div>
											</div>
												...
												{isLoading ? (
														...
														{[...Array(3)].map((e, index) => {
															return (
																...
																rounded-full bg-gray-200 text-gray-200"
																	...
																	<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-100">Loading..</span>
																</div>
															);
														})}
													</div>
													...
													<>
														...
														{availableTokens.tokens[mutation[key].gemBank.toBase58()]?.availableMints !== undefined &&
																...
																<div className="space-y-2">
																	{Object.keys(availableTokens.tokens[mutation[key].gemBank.toBase58()]?.availableMints).map((key_, index) => (
																		<div
																			...
																			className={`${selectedTokens[mutation[key].gemBank.toBase58()]?.mint === key_ ? "border-amber-500 border-8" : "border-gray-200"} 
															...
															: (
																<div>
																	<span className="text-gray-50 pl-1 ">NO TOKENS AVAILABLE FOR RECIPE</span>
																	{/* <input
