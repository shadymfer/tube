		...
		<div className="py-10">
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					<div className="space-y-1">
						{isLoading ? (
							...
							<span
								className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium ${transmuterOwner?.toBase58() === wallet.publicKey?.toBase58() ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
									}`}
							>
						...
						<div>
							<h2 className="text-lg font-medium text-gray-500 truncate ">TUBES</h2>
							<Transition
								show={showItem}
			...
			<main>
				<div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
					...
					<h2 className="text-xl font-bold leading-tight text-gray-700 my-4">Recipes</h2>
					{transmuterOwner?.toBase58() === wallet.publicKey?.toBase58() && (
						<button
							onClick={() => {
								toggleMutation();
						...
						<>
							{mutations.length > 0 ? (
									...
									{mutations.map((mutation, index) => (
										...
										>
											<div className="group block w-full aspect-w-10 aspect-h-7 rounded-lg bg-white shadow-sm   hover:opacity-75 focus-within:ring-2  transiton-all duration-150 ease-in focus-within:ring-offset-2 focus-within:ring-offset-gray-100 focus-within:ring-indigo-500 overflow-hidden">
												...
												<button type="button" className="absolute inset-0 focus:outline-none">
													<span className="sr-only">View details of the Recipe</span>
												</button>
											</div>
										</li>
									))}
								...
								>
									...
									<span className="mt-2 block text-sm font-medium text-gray-900">Make a new Recipe</span>
								</button>
							)}{" "}
						</>
					)}
